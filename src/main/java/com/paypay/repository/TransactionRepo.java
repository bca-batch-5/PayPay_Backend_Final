...
public interface TransactionRepo extends JpaRepository<Transaction, Integer> {
    ...
    @Query(value = "SELECT nominal FROM `transaction` WHERE transaction_type='Kredit' AND id_user=? LIMIT 1;", nativeQuery = true)
    Long getKredit(Integer idUser);

    @Query(value = "SELECT nominal FROM `transaction` WHERE transaction_type='Debit' AND id_user=? LIMIT 1;", nativeQuery = true)
